...
class Article(TimeStampedUUIDModel):
    ...
    title = models.CharField(max_length=250, verbose_name=_("user"))
    slug = AutoSlugField(populate_from = 'title', always_update=True, unique=True)
    description = models.CharField(verbose_name=_("description"), max_length=255)
    body = models.TextField(verbose_name=_("article content"), blank=True)
    ...
    def get_average_ratng(self):
        if Rating.objects.all().count() > 0:
            rating = (
                Rating.objects.filter(article=self.pkid).all().aggregrate(Avg('value'))
            )
...
class ArticleViews(TimeStampedUUIDModel):
    ...
    def __Str__(self):
        return (
            f"Total vews on - {self.article.title} is - {self.article.views} views(S)"
        )
